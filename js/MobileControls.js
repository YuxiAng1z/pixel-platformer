class MobileControls {
    constructor(scene) {
        ...
        this._joyPointerId = null; // Track which finger is on joystick
        this._setupJoystick();
        this._setupJumpBtn();
    }
    ...
    _setupJoystick() {
        ...
        this.joyBase = s.add.circle(bx, by, baseR, 0xffffff, 0.18)
            .setScrollFactor(0).setDepth(100);
        this.joyBase.setStrokeStyle(2, 0x00e5ff, 0.5);

        this.joyThumb = s.add.circle(bx, by, thumbR, 0x00e5ff, 0.65)
            .setScrollFactor(0).setDepth(101);

        // Joystick zone
        const zone = s.add.zone(bx, by, baseR * 2 + 40, baseR * 2 + 40)
            .setScrollFactor(0).setDepth(102).setInteractive();

        zone.on('pointerdown', (pointer) => {
            this._joyPointerId = pointer.id;
            this._updateJoy(pointer, bx, by, baseR);
        });
    ...
    _setupJumpBtn() {
        ...
        const btn = s.add.circle(bx, by, 45, 0xff6b35, 0.75)
        ...
        // Use separate pointer tracking for jump (won't conflict with joystick)
